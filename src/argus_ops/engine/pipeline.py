...
class CollectorCircuitBreaker:
    ...
    def __init__(self, name: str, failure_threshold: int = 3, reset_timeout: float = 60.0) -> None:
    ...
    def call(self, fn: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        if self._state == self._OPEN:
            ...
            if elapsed >= self._reset_timeout:
                ...
                logger.info("Circuit breaker '%s': OPEN -> HALF_OPEN (trial call)", self._name)
            else:
                raise CircuitOpen(
...
class Pipeline:
    ...
    def collect(self) -> list[HealthSnapshot]:
        ...
        for collector in self.collectors:
            ...
            try:
                ...
                logger.info("Collector %s returned %d snapshot(s)", collector.name, len(result))
            except CircuitOpen as exc:
                logger.warning("Collector %s skipped: %s", collector.name, exc)
        ...
        def _run() -> list[HealthSnapshot]:
            ...
            for snapshot in result:
                if not snapshot.capabilities:
                    snapshot.capabilities = list(collector.provided_capabilities)
            return result
    ...
    def analyze(self, snapshots: list[HealthSnapshot]) -> list[Finding]:
        ...
        for analyzer in self.analyzers:
            if analyzer.required_capabilities and not analyzer.supports(available_capabilities):
                logger.info(
                    "Skipping analyzer %s due to missing capabilities: %s",
                    analyzer.name,
                    ", ".join(analyzer.required_capabilities),
                )
                continue
            ...
            try:
                ...
                if result:
                    logger.info("Analyzer %s found %d issue(s)", analyzer.name, len(result))
            except Exception as exc:
                logger.error("Analyzer %s failed: %s", analyzer.name, exc)
    ...
    def diagnose(
        ...
        for group_name, group_findings in groups.items():
            if not group_findings:
                continue
            logger.info("Diagnosing group '%s' with %d finding(s)", group_name, len(group_findings))
            try:
                diagnosis = self.ai_provider.diagnose(group_findings, context or {})
                incident = Incident(
                ...
                logger.error("Diagnosis failed for group '%s': %s", group_name, exc)
                incidents.append(self._findings_to_incident(group_findings))

        return incidents

    def run_full(self, context: dict[str, Any] | None = None) -> list[Incident]:
    ...
    def _group_findings(findings: list[Finding]) -> dict[str, list[Finding]]:
        ...
        for finding in findings:
            groups.setdefault(finding.infra_type.value, []).append(finding)
        return groups

    @staticmethod
    def _findings_to_incident(findings: list[Finding]) -> Incident:
        return Incident(incident_id=f"INC-{uuid.uuid4().hex[:8]}", findings=findings)
