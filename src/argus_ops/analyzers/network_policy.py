...
class NetworkPolicyAnalyzer(BaseAnalyzer):
    ...
    - Deny-all policies with no explicit ingress rules
    """
    ...
    def analyze(self, snapshots: list[HealthSnapshot]) -> list[Finding]:
        ...
        for snap in snapshots:
            ...
            for np in network_policies:
                ...
                # Empty rules with a declared policy type create a deny-all baseline.
                if "Ingress" in policy_types and not ingress_rules:
                    findings.append(
                        Finding(
                            ...
                            title=f"Deny-all ingress policy: {np_name}",
                            description=(
            ...
            for ns_info in namespaces:
                ...
                if ns_name in _SYSTEM_NAMESPACES:
                    continue
                workload_namespaces.add(ns_name)
        ...
        total_ns = len(workload_namespaces)
        ...
        if total_ns > 0 and uncovered > total_ns * 0.5:
            findings.append(
                Finding(
                    ...
                    severity=Severity.MEDIUM,
                    title=(
                        f"Network policy coverage gap: {uncovered}/{total_ns} "
                        "namespaces unprotected"
                    ),
                    description=(
