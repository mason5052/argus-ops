    ...
    def analyze(self, snapshots: list[HealthSnapshot]) -> list[Finding]:
        ...
        for snap in snapshots:
            ...
            for pod in pods:
                ...
                if security_context.get("run_as_user") == 0:
                    findings.append(
                        Finding(
                            finding_id=f"SEC-{uuid.uuid4().hex[:8]}",
                            category=FindingCategory.SECURITY,
                            severity=Severity.MEDIUM,
                            title=f"Pod runs as root: {pod_name}",
                            description=(
                                f"Pod '{pod_name}' in namespace '{pod_ns}' sets "
                                "runAsUser=0 at the pod level."
                            ),
                            target=f"{pod_ns}/pod/{pod_name}",
                            infra_type=InfraType.KUBERNETES,
                ...
                if host_pid:
                    findings.append(
                        Finding(
                            ...
                            category=FindingCategory.SECURITY,
                            severity=Severity.MEDIUM,
                            title=f"hostPID enabled: {pod_name}",
                            description=(
                                f"Pod '{pod_name}' in namespace '{pod_ns}' uses "
                                "hostPID, sharing the node's process namespace."
                            ),
                            target=f"{pod_ns}/pod/{pod_name}",
                            infra_type=InfraType.KUBERNETES,
                ...
                if host_ipc:
                    findings.append(
                        Finding(
                            ...
                            category=FindingCategory.SECURITY,
                            severity=Severity.MEDIUM,
                            title=f"hostIPC enabled: {pod_name}",
                            description=(
                                f"Pod '{pod_name}' in namespace '{pod_ns}' uses "
                                "hostIPC, sharing the node's IPC namespace."
                            ),
                            target=f"{pod_ns}/pod/{pod_name}",
                            infra_type=InfraType.KUBERNETES,
